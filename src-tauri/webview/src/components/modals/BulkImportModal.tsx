...
const RadioOption = styled.label<{ $selected: boolean }>`
  ...
  cursor: pointer;
  background: ${(props) =>
    props.$selected ? "var(--apinox-list-hoverBackground)" : "transparent"};
...
export const BulkImportModal: React.FC<BulkImportModalProps> = ({
  ...
  const [importMode, setImportMode] = useState<ImportMode>(
    existingProjects.length > 0 ? "existing" : "new",
  );
  ...
  // Validation
  const projectName =
    importMode === "new" ? newProjectName.trim() : selectedProject;
  ...
  const handleStartImport = useCallback(async () => {
    ...
    for (let i = 0; i < urls.length; i++) {
      ...
      setResults([
        ...newResults,
        ...urls.slice(i + 1).map((u) => ({ url: u, success: false })),
      ]);
    }
  ...
  const renderInputView = () => (
    <Content>
      <Description>
        Enter one WSDL or OpenAPI URL per line. Each URL will be parsed and
    ...
    <ProgressContainer>
        ...
        <SummaryBox>
          ...
          <div className="stat">
            <span className="value">
              {results.reduce((acc, r) => acc + (r.interfaces?.length || 0), 0)}
            </span>
      ...
      <ResultsList>
        {results.map((result, index) => {
          ...
          else if (viewState === "processing" && index === currentIndex)
            status = "processing";
            ...
            <ResultItem key={index} $status={status}>
              ...
              {result.interfaces && (
                <span style={{ opacity: 0.7 }}>
                  ({result.interfaces.length} interface
                  {result.interfaces.length !== 1 ? "s" : ""})
                </span>
              )}
              {result.error && (
