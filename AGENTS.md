...
5.  **Rust re-serializes the whole config on save**: `save_settings` (settings_manager.rs) deserializes into the typed `ApinoxConfig`/`UiConfig` structs and re-serializes them — any config field missing from those structs is **silently dropped from config.jsonc on every save**. When adding a new config key, add it to the TypeScript model (`shared/src/models.ts`), `UiConfig` + `Default` + the `update_ui_settings` merge in `settings_manager.rs`, and the Settings tab together, or it will vanish.
6.  **Content-Type header lock is a setting**: `config.ui.contentTypeLocked` (default `true`). When `false`, the request editor's Content-Type row is an editable override written into the request's `headers` (`Content-Type` entry), which execution honors first (`headers['content-type']` wins in `unifiedExecute` and `resolveEffectiveContentType`).
7.  **Skeletons must carry the stable request `id`**: The unified explorer's first paint is a SKELETON (`list_unified_projects_skeleton`) that omits request bodies; selecting a node upgrades that project to full detail (`load_unified_project_detail`). Sidebar rows select by `req.id || req.name`, and stored requests have a stable UUID `id` (not their name). If the skeleton drops that `id`, a request clicked while the project is still a skeleton (id = name) stops matching after the full detail lands (id = UUID) and the selection reverts to the empty state on first click. So `requestNames[]` entries in the skeleton (and the webview's skeleton→state map in `UnifiedProjectContext`) MUST include the request's `id`.
8.  **Test suites are GLOBAL, not per-project**: Test suites live in a single `~/.apinox/test-suites.json` (see `test_suite_storage.rs`), owned by the webview `TestSuiteContext` (`useTestSuites` / `useTestSuitesOptional`). They are project-agnostic because a `TestStep` embeds a full `ApiRequest` copy in `config.request` (self-contained; never resolves against a project) and `workflowId` resolves against the global `workflows` list. The historical per-project `tests/` layout is migrated once at startup by `migrate_all_project_suites_to_global` (idempotent by suite id) and `save_unified_project` no longer writes `tests/` — it instead merges any in-memory `project["testSuites"]` into the global store (`merge_project_suites_into_global`, for the legacy-migration + import flows) and removes the dir. So: create a new suite via `addSuite` (no project picker), read the list from `useTestSuites()`, and persist step edits through `updateTestCase` — do NOT write suites onto `UnifiedProject.testSuites`.
