 ...
 */
export const renameInFolders = (folders, targetId, targetType, newName) => {
    let updated = false;
    const newFolders = folders.map(folder => {
        ...
        if (targetType === 'request') {
            ...
            if (reqIndex !== -1) {
                updated = true;
                requests = folder.requests.map((r, i) => i === reqIndex ? { ...r, name: newName, dirty: true } : r);
            }
        }
 ...
 */
export const updateProjectWithRename = (projects, targetId, // ID or Name depending on what's available context
targetType, newName, targetData // Fallback if we need to match by reference
) => {
            ...
            newInterfaces = p.interfaces.map(i => ({
                ...
                operations: i.operations.map(o => {
                    ...
                    if (reqIndex !== -1) {
                        ...
                        return {
                            ...o,
                            requests: o.requests.map((r, idx) => idx === reqIndex ? { ...r, name: newName, dirty: true } : r)
                        };
                    }
                    return o;
                })
 ...
 */
const findPathInFolders = (folders, targetId) => {
 ...
 */
export const findPathToRequest = (projects, targetId) => {
