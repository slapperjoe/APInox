 ...
 */
export const generateXmlFromSchema = (operationName, inputSchema, targetNamespace) => {
    // Helper to recursively build body content
    const buildBody = (node, indent = '') => {
        if (!node)
            return '';
 ...
 */
export const getInitialXml = (input, indent = '         ') => {
    if (!input || typeof input !== 'object') {
        return '';
    }
    const lines = [];
    // Recursively build XML for each parameter
    const buildXml = (obj, currentIndent) => {
