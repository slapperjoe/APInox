 ...
 */
const extractSoapOperationName = (requestBody) => {
    if (!requestBody)
        return null;
    try {
 ...
 */
const extractSoapTextParameters = (content) => {
    if (!content)
        return [];
    const params = [];
    const seen = new Set();
    try {
        ...
        while ((match = paramRegex.exec(content)) !== null) {
            ...
            if (/^\d+$/.test(textContent))
                continue;
            if (['true', 'false', 'null', 'undefined'].includes(textContent.toLowerCase()))
                continue;
...
export const createMockRuleFromSource = (data) => {
