...
export function ProjectProvider({ children, initialProjects = [] }) {
        ...
        const timer = setTimeout(() => {
            if (projects.length > 0) {
                ...
                const openProjectPaths = projects
                    .filter(p => p.fileName && p.name !== 'Samples' && p.id !== 'samples-project-read-only')
                    .map(p => p.fileName);
     ...
     */
    const closeProject = useCallback((name) => {
    ...
    const saveProject = useCallback(async (project) => {
        debugLog('saveProject', { name: project.name, hasPath: !!project.fileName });
        // If this is a new project (no file path) and we are in Tauri, prompt for location
        if (!project.fileName && bridge.isTauri()) {
            try {
            ...
            switch (message.command) {
                ...
                case BackendCommand.ProjectLoaded:
                    ...
                    if (message.project) {
                        ...
                        setProjects(prev => {
                            const existingIndex = prev.findIndex(p => (p.id && p.id === message.project.id) || p.name === message.project.name);
                            if (existingIndex !== -1) {
                                // Update existing
                                const updated = [...prev];
     ...
     */
    const toggleProjectExpand = useCallback((name) => {
     ...
     */
    const toggleInterfaceExpand = useCallback((projectName, interfaceName) => {
        setProjects(prev => prev.map(p => {
            if (p.name !== projectName)
                return p;
            return {
                ...p,
                interfaces: p.interfaces.map(i => i.name === interfaceName ? { ...i, expanded: !i.expanded } : i)
            };
        }));
    }, []);
    /**
     ...
     */
    const toggleOperationExpand = useCallback((projectName, interfaceName, operationName) => {
        setProjects(prev => prev.map(p => {
            if (p.name !== projectName)
                return p;
            return {
                ...
                interfaces: p.interfaces.map(i => {
                    ...
                    return {
                        ...i,
                        operations: i.operations.map(o => o.name === operationName ? { ...o, expanded: !o.expanded } : o)
                    };
                })
            };
        }));
     ...
     */
    const updateProject = useCallback((name, updater) => {
        setProjects(prev => prev.map(p => {
            if (p.name === name) {
     ...
     */
    const findProjectByName = useCallback((name) => {
