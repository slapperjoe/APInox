    ...
    return (
        <ScrollableForm>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                ...
                {config.fileWatcher !== undefined || true ? (
                    <div>
                        <SectionHeader style={{ marginTop: 0 }}>File Watcher (Hot Reload)</SectionHeader>
                        <div style={{ display: 'flex', gap: '20px' }}>
                            <FormGroup style={{ flex: 1 }}>
                                <Label>Request XML Path (Default: C:\temp\requestXML.xml on Windows)</Label>
                                <Input
                                    type="text"
                                    placeholder="Absolute path to request file"
                                    value={config.fileWatcher?.requestPath ?? ''}
                                    onChange={e => onChange('fileWatcher', 'requestPath', e.target.value)}
                                />
                            </FormGroup>
                            <FormGroup style={{ flex: 1 }}>
                                ...
                                <Input
                                    type="text"
                                    placeholder="Absolute path to response file"
                                    value={config.fileWatcher?.responsePath ?? ''}
                                    onChange={e => onChange('fileWatcher', 'responsePath', e.target.value)}
                                />
                            </FormGroup>
                        </div>
                    </div>
