...
`;
export const StatusCodePicker = ({ value, onChange }) => {
    ...
    return (_jsxs(Container, { ref: containerRef, children: [_jsxs(PillsContainer, { children: [selectedCodes.map(code => (_jsxs(Pill, { "$isError": isErrorCode(code), children: [code, _jsx(RemoveButton, { onClick: () => removeCode(code), title: "Remove", children: _jsx(X, { size: 10 }) })] }, code))), _jsxs(DropdownTrigger, { ref: triggerRef, onClick: () => setIsOpen(!isOpen), children: ["+ Add ", _jsx(ChevronDown, { size: 12 })] })] }), isOpen && createPortal(_jsx(Dropdown, { style: { top: dropdownPos.top, left: dropdownPos.left }, children: COMMON_CODES.map(({ code, label }) => (_jsxs(DropdownItem, { "$isSelected": selectedCodes.includes(code), onClick: () => addCode(code), children: [label, selectedCodes.includes(code) && _jsx("span", { children: "\u2713" })] }, code))) }), document.body)] }));
};
