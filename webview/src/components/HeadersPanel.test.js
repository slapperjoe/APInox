...
vi.mock('../components/MonacoSingleLineInput', () => ({
    MonacoSingleLineInput: ({ value, onChange, placeholder }) => (_jsx("input", { "data-testid": `input-${placeholder}`, value: value, onChange: (e) => onChange(e.target.value), placeholder: placeholder }))
}));
describe('HeadersPanel', () => {
    it('should render empty state when no headers', () => {
        const onChange = vi.fn();
        render(_jsx(HeadersPanel, { headers: {}, onChange: onChange }));
        expect(screen.getByText('No custom headers defined.')).toBeInTheDocument();
    });
    it('should render headers', () => {
        const onChange = vi.fn();
        const headers = { 'Authorization': 'Bearer token123' };
        render(_jsx(HeadersPanel, { headers: headers, onChange: onChange }));
    ...
    it('should render Content-Type as read-only', () => {
        const onChange = vi.fn();
        render(_jsx(HeadersPanel, { headers: {}, onChange: onChange, contentType: "application/xml" }));
