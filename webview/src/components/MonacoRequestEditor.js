...
const EditorContainer = styled.div `
  ...
  overflow: hidden;
`;
export const MonacoRequestEditor = forwardRef(({ value, onChange, language = 'xml', readOnly = false, onFocus, autoFoldElements, requestId, forceUpdateKey }, ref) => {
    const editorRef = useRef(null);
    const monacoRef = useRef(null);
    const previousRequestIdRef = useRef(undefined);
    const lastSyncedRequestIdRef = useRef(undefined);
    const lastSyncedForceUpdateKeyRef = useRef(undefined);
    ...
    return (_jsxs(EditorContainer, { children: [_jsx("style", {}), _jsx(Editor, { height: "100%", defaultLanguage: language, defaultValue: value, onChange: (val) => onChange(val || ''), theme: "vs-dark" // Default to dark, ideally sync with VSCode theme
                , onMount: handleEditorDidMount, options: editorOptions })] }));
});
