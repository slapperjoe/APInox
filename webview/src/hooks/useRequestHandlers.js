...
export function useRequestHandlers({ selectedRequest, setSelectedRequest, selectedOperation, selectedInterface, selectedTestCase, selectedStep, testExecution, wsdlUrl, setLoading, setResponse, setWorkspaceDirty, setExploredInterfaces, }) {
                ...
                const updatedProjects = prev.map(p => {
                    ...
                    const updatedProject = {
                        ...
                        interfaces: p.interfaces.map(i => {
                            if (i.name !== selectedInterface?.name)
                                return i;
                            return {
                                ...i,
...
function updateFolderRequest(folders, requestId, updated) {
    return folders.map(folder => ({
        ...folder,
        requests: folder.requests.map((r) => (r.id === requestId || r.name === requestId) ? updated : r),
...
export function getInitialXml(input) {
    if (!input)
        return '';
