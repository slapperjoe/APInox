...
export function useSidebarCallbacks({ projects, setProjects, deleteConfirm, setDeleteConfirm, saveProject, setWatcherRunning, setWatcherHistory, setProxyRunning, setProxyHistory, proxyConfig, setProxyConfig, configPath, config }) {
    const handleAddSuite = useCallback((projName) => {
    ...
    const handleDeleteSuite = useCallback((suiteId) => {
        if (deleteConfirm === suiteId) {
            setProjects(prev => prev.map(p => {
                if (!p.testSuites || !p.testSuites.some(s => s.id === suiteId))
                    return p;
    ...
    const handleToggleSuiteExpand = useCallback((suiteId) => {
        setProjects(prev => prev.map(p => {
            if (!p.testSuites?.some(s => s.id === suiteId))
                return p;
    ...
    const handleToggleCaseExpand = useCallback((caseId) => {
        setProjects(prev => prev.map(p => {
    ...
    const handleAddTestCase = useCallback((suiteId) => {
        setProjects(prev => prev.map(p => {
    ...
    const handleRenameTestCase = useCallback((caseId, newName) => {
        ...
        setProjects(prev => {
            ...
            return prev.map(p => {
                ...
                const updatedSuite = {
                    ...suite,
                    testCases: suite.testCases?.map(tc => tc.id === caseId ? { ...tc, name: newName } : tc) || []
                };
                const updatedProject = {
                    ...p,
                    testSuites: p.testSuites.map(s => s.id === suite.id ? updatedSuite : s),
    ...
    const handleRenameTestStep = useCallback((caseId, stepId, newName) => {
        setProjects(prev => {
            return prev.map(p => {
                const suite = p.testSuites?.find(s => s.testCases?.some(tc => tc.id === caseId));
                if (!suite)
    ...
    const handleUpdateProxyConfig = useCallback((newConfig) => {
