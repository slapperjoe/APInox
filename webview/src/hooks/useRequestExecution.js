...
export function useRequestExecution({ selectedOperation, selectedRequest, selectedInterface, selectedTestCase, selectedStep, selectedProjectName, wsdlUrl, setLoading, setResponse, setSelectedRequest, setProjects, setWorkspaceDirty, testExecution, selectedPerformanceSuiteId, config, setConfig }) {
    const startTimeRef = useRef(0);
    const executeRequest = useCallback((xml) => {
        ...
        if (selectedOperation || selectedRequest) {
            ...
            const logToOutput = (msg) => bridge.sendMessage({ command: 'log', message: msg });
                    ...
                    priorSteps.forEach(step => {
                        if (step.type === 'request' && step.config.request?.extractors) {
                            ...
                            step.config.request.extractors.forEach(ext => {
                                ...
                                if (contextVariables[ext.variable])
                                    return;
    ...
    const projectUpdateTimer = useRef(null);
        ...
        if (selectedPerformanceSuiteId && updated.id?.startsWith('perf-req-')) {
            ...
            if (setConfig && config) {
                setConfig((prev) => {
                    ...
                    const suiteIndex = suites.findIndex((s) => s.id === selectedPerformanceSuiteId);
                    if (suiteIndex === -1)
                        return prev;
                    // ... verify structure ... 
                    const suite = { ...suites[suiteIndex] };
                    const reqIndex = suite.requests.findIndex((r) => r.id === updated.id);
                    if (reqIndex !== -1) {
                        const updatedReq = {
                ...
                const updatedProjects = prev.map(p => {
                    ...
                    if (selectedTestCase) {
                        ...
                        const updatedSuites = p.testSuites?.map(s => {
                            ...
                            const stepIndex = updatedCases[tcIndex].steps.findIndex(step => (updated.id && step.config.request?.id === updated.id) ||
                    ...
                    if (p.name !== selectedProjectName && !selectedTestCase)
                        return p;
...
function updateFolderRequestInExecution(folders, requestId, updated, onFound) {
    return folders.map(folder => {
        let foundInThis = false;
        const updatedRequests = folder.requests.map((r) => {
