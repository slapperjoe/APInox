...
export const useAppLifecycle = ({ projects, exploredInterfaces, explorerExpanded, wsdlUrl, selectedProjectName, saveProject, setProjects, setExploredInterfaces, setExplorerExpanded, setWsdlUrl, setSelectedProjectName }) => {
    ...
    useEffect(() => {
        ...
        if (state) {
            ...
            if (state.lastSelectedProject)
                setSelectedProjectName(state.lastSelectedProject);
    ...
    useEffect(() => {
        ...
        const paths = projects.map(p => p.fileName).filter(Boolean);
