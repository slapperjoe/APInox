...
export function useMessageHandler(state) {
    const { setProjects, setExploredInterfaces, setExplorerExpanded, setLoading, setResponse, setDownloadStatus, setSelectedFile, setSampleModal, setBackendConnected, setConfig, setRawConfig, setLayoutMode, setShowLineNumbers, setSplitRatio, setInlineElementValues, setConfigPath, 
    // setProxyConfig,
    setSelectedProjectName, setWsdlUrl, setWorkspaceDirty, setSavedProjects, setChangelog, setWatcherHistory, 
    // setProxyHistory,
    // setProxyRunning,
    // setTestExecution,
    setActiveView, setActiveBreakpoint, 
    // setMockHistory,
    // setMockRunning,
    // setMockConfig,
    // setActiveRunId,
    ...
    setRequestHistory, wsdlUrl, projects, config, selectedTestCase, selectedRequest, startTimeRef, saveProject, onAttachmentSelected, setWsdlDiff } = state;
            ...
            switch (message.command) {
                case BackendCommand.WsdlParsed:
                    ...
                    if (Array.isArray(data)) {
                        ...
                        data.forEach((svc) => {
                            ...
                            const operationsByPort = new Map();
                            svc.operations.forEach((op) => {
                ...
                case BackendCommand.ProjectLoaded:
                    ...
                    if (message.project?.testSuites) {
                        message.project.testSuites.forEach((ts) => {
                ...
                case BackendCommand.WorkspaceLoaded:
                    ...
                    setProjects(message.projects.map((p) => ({ ...p, expanded: false })));
                ...
                case BackendCommand.SettingsUpdate:
                    ...
                    if (message.config.ui) {
                        if (message.config.ui.layoutMode)
                            setLayoutMode(message.config.ui.layoutMode);
                        if (message.config.ui.showLineNumbers !== undefined)
                            setShowLineNumbers(message.config.ui.showLineNumbers);
                        if (message.config.ui.splitRatio !== undefined)
                            setSplitRatio(message.config.ui.splitRatio);
                        if (message.config.ui.inlineElementValues !== undefined)
                            setInlineElementValues(message.config.ui.inlineElementValues);
                    }
                    ...
                    if (!hasPerformedInitialLoad.current && message.config.openProjects && message.config.openProjects.length > 0) {
                        ...
                        message.config.openProjects.forEach((path) => {
                        ...
                        try {
                            ...
                            setProjects(prev => {
                                ...
                                savedProjects.forEach((saved) => {
                                    const existingIdx = merged.findIndex(p => (p.id && p.id === saved.id) || p.name === saved.name);
                                    if (existingIdx >= 0) {
                            ...
                            if (savedState.lastSelectedProject)
                                setSelectedProjectName(savedState.lastSelectedProject);
