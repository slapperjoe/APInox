...
describe('Request Update Routing', () => {
    ...
    const createMockRequest = (id, name) => ({
        id,
        name,
    ...
    const createMockProject = (requests) => ({
        name: 'Test Project',
        id: 'proj-1',
        interfaces: [{
                name: 'TestInterface',
    ...
    const createMockTestCase = (stepRequests) => ({
        id: 'tc-123',
        name: 'Test Case 1',
    ...
    describe('Performance Request Detection', () => {
        it('should identify performance requests by ID prefix "perf-req-"', () => {
            ...
            const isPerformanceRequest = (req) => req.id?.startsWith('perf-req-');
        ...
        it('should handle requests without ID gracefully', () => {
            ...
            const isPerformanceRequest = (req) => req.id?.startsWith('perf-req-') ?? false;
    ...
    describe('Test Case Request Detection', () => {
        it('should detect if request is in selected test case by ID match', () => {
            ...
            const isRequestInTestCase = testCase.steps.some(step => step.config.request?.id === request.id);
        ...
        it('should detect if request is in selected test case by name match', () => {
            ...
            const isRequestInTestCase = testCase.steps.some(step => step.config.request?.name === updatedRequestWithDifferentId.name);
            expect(isRequestInTestCase).toBe(true);
        });
        it('should return false when request is NOT in selected test case', () => {
            ...
            const isRequestInTestCase = testCase.steps.some(step => step.config.request?.id === workspaceRequest.id ||
        ...
        it('should fall through to workspace path when request not in stale test case', () => {
            ...
            const isRequestInTestCase = staleSelectedTestCase.steps.some(step => step.config.request?.id === workspaceRequest.id ||
                step.config.request?.name === workspaceRequest.name);
        ...
        it('should update request when project name matches', () => {
            ...
            const updatedProject = {
                ...
                interfaces: project.interfaces.map(i => {
                    if (i.name !== selectedInterfaceName)
                        return i;
                    return {
                        ...i,
                        operations: i.operations.map(o => {
                            if (o.name !== selectedOperationName)
                                return o;
                            return {
                                ...o,
                                requests: o.requests.map(r => r.id === updatedRequest.id ? updatedRequest : r)
                            };
                        })
                    };
                })
    ...
    describe('Stale State Scenarios', () => {
        ...
        it('scenario: edit Test Case step request (should still work)', () => {
            ...
            const isRequestInTestCase = selectedTestCase.steps.some(step => step.config.request?.id === stepRequest.id);
