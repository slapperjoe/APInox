...
export class ExecuteRequestCommand implements ICommand {
  ...
  async execute(message: any): Promise<void> {
    try {
      ...
      const activeEnv = config.environments
        ? config.environments[activeEnvName]
        : {};
      ...
      const processedUrl = WildcardProcessor.process(
        message.url,
        envVars,
        globals,
        scriptsDir,
      ...
      let processedXml = WildcardProcessor.process(
        message.xml,
        envVars,
        globals,
        scriptsDir,
      ...
      if (wsSecurity && wsSecurity.type !== WSSecurityType.None) {
        ...
        if (wsSecurity.type === WSSecurityType.UsernameToken) {
          processedXml = WSSecurityUtil.applyToRequest(
            processedXml,
            processedWsSecurity,
          );
          ...
          if (
            !processedWsSecurity.privateKeyPath ||
            !processedWsSecurity.publicCertPath
          ) {
            throw new Error(
              "Certificate authentication requires both private key and public certificate paths",
            );
          }
          ...
          processedXml = WSSecurityUtil.applyCertificateToRequest(
            processedXml,
            {
              privateKey,
              publicCert,
      ...
      if (attachments && attachments.length > 0) {
        ...
        processedXml = AttachmentUtil.inlineBase64Attachments(
          processedXml,
          attachments,
        );
        ...
        if (AttachmentUtil.hasMultipartAttachments(attachments)) {
          const result = AttachmentUtil.processAttachments(
            processedXml,
            attachments,
          );
          ...
          this._soapClient.log(
            `SwA multipart request prepared with ${attachments.filter((a) => a.type === "SwA" || a.type === "MTOM").length} attachment(s)`,
          );
        }
      }
      ...
      if (headers) {
        ...
        for (const key in headers) {
          if (Object.prototype.hasOwnProperty.call(headers, key)) {
            processedHeaders[key] = WildcardProcessor.process(
              headers[key],
              envVars,
              globals,
              scriptsDir,
      ...
      this._soapClient.log(
        `[ExecuteRequest] RequestType: ${requestType} (Original: ${message.requestType || "undefined"})`,
      );
      this._soapClient.log(
        `[ExecuteRequest] XML/Body: ${processedXml.substring(0, 100)}${processedXml.length > 100 ? "..." : ""}`,
      );
      ...
      if (requestType === "rest" || requestType === "graphql") {
        ...
        const request: SoapUIRequest = {
          ...
          method:
            message.method || (requestType === "graphql" ? "POST" : "GET"),
          headers: headers,
          requestType: requestType,
          bodyType:
            message.bodyType ||
            (requestType === "graphql" ? "graphql" : "json"),
        ...
        if (isMultipart && formData) {
          ...
          soapRequest = {
            ...
            headers: soapHeaders,
            requestType: "soap",
            contentType: soapHeaders["Content-Type"],
          };
        }

        result = await this._httpClient.execute(soapRequest);
      }
      ...
      if (message.assertions && Array.isArray(message.assertions)) {
        ...
        assertionResults = AssertionRunner.run(
          typeof result === "string"
            ? result
            : result?.rawResponse || JSON.stringify(result),
          timeTaken,
          message.assertions,
          statusCode,
        );
