...
export function getInstalledFonts(): MonoFont[] {
  const installed = MONOSPACE_FONTS.filter((font) => {
    ...
    // Debug logging
    debugLog(
      `[FontDetection] ${font.name} (${primaryFontName}): ${isInstalled ? "✓ installed" : "✗ not found"}`,
    );
  ...
  debugLog(
    `[FontDetection] Found ${installed.length}/${MONOSPACE_FONTS.length} installed fonts:`,
    installed.map((f) => f.name).join(", "),
  );
