...
export function findElementRanges(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (openMatch && !line.includes("</")) {
      ...
      if (
        elementNames.some(
          (name) => tagName.toLowerCase() === name.toLowerCase(),
        )
      ) {
...
export function applyAutoFolding(
  ...
  setTimeout(() => {
    ...
    try {
      ...
      if (
        foldingController &&
        typeof foldingController.getFoldingModel === "function"
      ) {
        foldingController
          .getFoldingModel()
          .then((foldingModel: any) => {
            if (!foldingModel) {
              onComplete?.();
              return;
            }
            ...
            ranges.forEach((r) => {
              try {
                ...
                debugError(
                  `[xmlFoldingUtils] Error finding region at line ${r.startLine}`,
                  e,
                );
              }
            });
            ...
            if (regionsToCollapse.length > 0) {
              ...
              try {
                ...
                try {
                  ...
                  debugError("[xmlFoldingUtils] Folding fallback failed", e2);
                }
              }
            }
          ...
          .catch((err: any) => {
            debugError("[xmlFoldingUtils] Error getting folding model", err);
            onComplete?.();
          });
      } else {
      ...
      debugError("[xmlFoldingUtils] Error during folding", error);
      onComplete?.();
    }
