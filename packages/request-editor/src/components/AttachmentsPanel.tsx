...
const DropZone = styled.div<{ isDragging?: boolean }>`
  ...
  color: var(--apinox-descriptionForeground);
  background: ${(props) =>
    props.isDragging ? "var(--apinox-list-hoverBackground)" : "transparent"};
...
export const AttachmentsPanel: React.FC<AttachmentsPanelProps> = ({
  ...
  const handleAddClick = useCallback(() => {
    // TODO: Request file picker from backend
    // bridge.sendMessage({ command: FrontendCommand.SelectAttachment });
    debugLog(
      "[AttachmentsPanel] File picker not yet implemented in standalone package",
    );
  }, []);
  ...
  const handleUpdateContentId = useCallback(
        ...
        <AttachmentList>
          {attachments.map((att) => (
            <AttachmentCard key={att.id}>
              <FileIcon>{getFileIcon(att.contentType)}</FileIcon>
              <FileInfo>
              ...
              <TypeSelect
                value={att.type}
                onChange={(e) =>
                  handleUpdateType(att.id, e.target.value as AttachmentType)
                }
                title="Encoding type"
              >
