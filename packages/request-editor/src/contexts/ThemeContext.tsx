...
export const ThemeProvider = ({
  ...
  useEffect(() => {
    ...
    debugLog(
      `[ThemeContext] Applied ${theme} theme (${Object.keys(selectedTheme.variables).length} variables)`,
    );
    ...
    const updateBorderColor = async () => {
      ...
      try {
        ...
        debugLog(`[ThemeContext] Updated border color to: ${editorBg}`);
      } catch (e) {
        debugWarn("[ThemeContext] Failed to update border color", e);
      }
    };
  ...
  const setTheme = (newTheme: ThemeName) => {
    if (!isStandalone) {
      debugWarn("[ThemeContext] Theme switching disabled in embedded mode");
      return;
    }
  ...
  return (
    <ThemeContext.Provider
      value={{ theme, setTheme, isStandalone, monacoTheme }}
    >
      {children}
    </ThemeContext.Provider>
  );
